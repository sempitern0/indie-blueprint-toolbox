...
static func parse_json(path: String, encrypted_key: String = "") -> Variant:
	if not path.get_extension() == "json":
		push_error("IndieBlueprintFileHelper: The file path %s provided does not have a valid JSON extension" % path)
		return {}
	...
	if open_error != OK:
		push_error("IndieBlueprintFileHelper: An error happened opening json file %s, Error %d-%s" % [path, open_error, error_string(open_error)])
		return {}
	...
	if json_error == OK:
		assert(json.data is Array or json.data is Dictionary, "IndieBlueprintFileHelper: Invalid data type, only JSON dictionary or arrays are supported")
	else:
		push_error("IndieBlueprintFileHelper: JSON Parse Error: ", json.get_error_message(), " in ", json_string_data, " at line ", json.get_error_line())
