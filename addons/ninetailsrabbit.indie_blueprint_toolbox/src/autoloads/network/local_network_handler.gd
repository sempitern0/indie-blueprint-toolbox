...
func start_broadcast(broadcast_port: int = DefaultBroadcastPort, dest_port: int = DefaultBroadcastListenPort, bind_address: String = "0.0.0.0") -> void:
	...
	if binded_port_error == OK:
		print("LocalNetworkHandler: Broadcast port %d binded with success " % broadcast_port)
	else:
		push_error("LocalNetworkHandler: An error %s happened when binding port on broadcast %d" % [error_string(binded_port_error), broadcast_port])
...
func start_broadcast_listener(listen_port: int = DefaultBroadcastListenPort, bind_address: String = "0.0.0.0") -> PacketPeerUDP:
	...
	if binded_port_error == OK:
		print("LocalNetworkHandler: Listener broadcast port %d binded with success " % listen_port)
	else:
		push_error("LocalNetworkHandler: An error %s happened when binding port on broadcast listener %d" % [error_string(binded_port_error), listen_port])
