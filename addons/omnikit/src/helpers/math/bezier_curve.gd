class OmniKitBezierCurve:
	
	static var maximum_control_points: int = 18
	static var factorial: Array[float] = OmniKitMathHelper.factorials_from(maximum_control_points)
	...
	static func point_3(t: float, control_points: Array[Vector3] = []) -> Vector3:
		...
		if n > maximum_control_points:
			push_warning("OmniKitBezierCurve: There more than %d control points (%d) in this bezier curve calculation, these points will be ignored" % [maximum_control_points, n])
